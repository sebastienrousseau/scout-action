...
# version. This one wraps scout's release, so its version is scout's latest
# release, exactly.
#
#   scripts/lockstep.sh
set -euo pipefail
...
if [ "$mine" = "$theirs" ]; then
  ...
  echo "lockstep: this repository is at $mine and scout's latest release is $theirs; run the sync workflow, or wait for the release to dispatch it" >&2
  exit 1
fi
